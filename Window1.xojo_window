...
#tag WindowCode
	...
	#tag Method, Flags = &h0
		Function SHA_StringFilter(s As String, addSpace As integer = -1) As String
		  Dim i As Integer
		  Dim a As String
		  Dim r As String
		  ...
		  For i = 1 To len(s)
		    ...
		    if (f.IndexOf(a) = -1) then
		      ...
		      c = c + 1
		      if (addSpace>=1 And  c>=addspace) then
		        r = r + " "
		        c = 0
		      end if
...
#tag Events PushButtonSHA
	#tag Event
		Sub Action()
		  ...
		  //s = Crypto.Hash(s, Crypto.HashAlgorithms.MD5)
		  ...
		  msgbox Str(dd.TotalSeconds-d.TotalSeconds) + " seconds"
		End Sub
	#tag EndEvent
#tag EndEvents
#tag ViewBehavior
